...
export class MyStock implements OnInit {
  ...
  submitBulkReclamation(): void {
    ...
    const promises = this.selectedStockItemsForReclamation.map(item => {
      return this.reclamationService.analyzeAndCreate(
        item.idStock || null,
    ...
    Promise.all(promises)
      .then(results => {
        ...
        alert(`✅ ${successCount} reclamation(s) submitted successfully!`);
      ...
      .catch(error => {
        ...
        alert('❌ Failed to submit reclamations. Please try again.');
      })
      .finally(() => {
