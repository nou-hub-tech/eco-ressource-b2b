...
export class Dashboard implements OnInit, AfterViewInit, OnDestroy {
  ...
  // ✅ Guard : retourne null si heroSlides est vide
  ...
  private loadDashboardData(): void {
    this.listingService.getAllListings().subscribe(listings => {
      ...
      // ✅ Fix NG0100 : force la détection après chargement async
  ...
  private updateCategoriesCount(listings: ListingDto[]): void {
    ...
    categories.forEach((cat, index) => {
      this.categories[index + 1].count = listings.filter(l => l.category === cat).length;
    });
  }
  ...
  private updateTicker(listings: ListingDto[]): void {
    ...
    if (metalListings.length > 0) {
      const avg = metalListings.reduce((s, l) => s + l.price, 0) / metalListings.length;
      this.ticker[0].price = avg.toFixed(0);
      this.ticker[2].price = (avg * 0.65).toFixed(0);
    }
    if (plasticListings.length > 0) {
      const avg = plasticListings.reduce((s, l) => s + l.price, 0) / plasticListings.length;
      this.ticker[1].price = avg.toFixed(0);
      this.ticker[7].price = (avg * 0.76).toFixed(0);
    }
  ...
  private getCategoryColor(category: string): string {
    const colors: { [key: string]: string } = {
      'Metal': '#0056d2', 'Plastic': '#0a7c4f', 'Paper': '#92400e',
      'Glass': '#4c1d95', 'Textile': '#a78bfa', 'Chemical': '#dc2626'
    };
  ...
  private getCategoryTag(category: string): string {
    const tags: { [key: string]: string } = {
      'Metal': 'PREMIUM · HIGH GRADE',     'Plastic': 'RECYCLED · ECO-FRIENDLY',
      'Paper': 'COMPRESSED · READY TO SHIP','Glass': 'CLEAN · FOOD GRADE',
      'Textile': 'SORTED · QUALITY ASSURED','Chemical': 'LAB TESTED · CERTIFIED'
    };
  ...
  private getCategoryAccent(category: string): string {
    const accents: { [key: string]: string } = {
      'Metal': 'Grade A', 'Plastic': 'Pellets', 'Paper': 'Bales',
      'Glass': 'Cullet',  'Textile': 'Bales',   'Chemical': 'Pure'
    };
