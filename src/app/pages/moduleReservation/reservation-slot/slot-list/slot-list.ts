...
export class SlotList implements OnInit {
  ...
  get filtered(): ReservationSlot[] {
    ...
    const list = this.slots
      .filter(s => !s.deleted)
      .filter(s => !q || s.machine.toLowerCase().includes(q) || s.date.toLowerCase().includes(q))
  ...
  get totalSlots(): number {
    return this.slots.filter(s => !s.deleted).length;
  }

  get openSlots(): number {
    return this.slots.filter(s => !s.deleted && s.status === 'OPEN').length;
  }

  get bookedSlots(): number {
    return this.slots.filter(s => !s.deleted && s.status === 'BOOKED').length;
  }

  get solarSlots(): number {
    return this.slots.filter(s => !s.deleted && s.solar).length;
  }
  ...
  get avgDiscount(): number {
    ...
    return Math.round(withDiscount.reduce((sum, slot) => sum + slot.discountPct, 0) / withDiscount.length);
  }
  ...
  goCreate(): void {
    this.router.navigate(['/enterprise/new-slot']);
  }

  goEdit(s: ReservationSlot): void {
    this.router.navigate(['/enterprise/edit-slot', s.id]);
  }
