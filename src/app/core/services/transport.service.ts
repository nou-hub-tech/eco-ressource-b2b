...
export class TransportService {
  ...
  // CORRECTION: Changement de l'URL
  getAllTransporters(): Observable<Transporter[]> {
    // Avant: ${this.apiUrl}/transport/transporters
    // Après: ${this.apiUrl}/transporters
    return this.http
      .get<TransporterApi[]>(`${this.apiUrl}/transporters`)  // ← URL CORRIGÉE
      .pipe(
        map((list) =>
          (list || [])
            .map((t) => {
